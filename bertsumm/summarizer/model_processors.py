...
class ModelProcessor(object):
    ...
    def _set_num_clusters(sentences, method):
        if method == "s_score":
            sc_scores = []
            for t in range(2, 11):
                kmeans_model = KMeans(n_clusters=t).fit(sentences)  # 训练模型
                sc_score = silhouette_score(sentences, kmeans_model.labels_, metric='euclidean')
                sc_scores.append(sc_score)
            print(sc_scores)
            return np.argmax(sc_scores) + 1
        if method == "c_score":
            sc_scores = []
            for t in range(2, 6):
                kmeans_model = KMeans(n_clusters=t).fit(sentences)  # 训练模型
                sc_score = calinski_harabasz_score(sentences.toarray(), kmeans_model.labels_)
                sc_scores.append(sc_score)
            print(sc_scores)
            return np.argmax(sc_scores) + 2
