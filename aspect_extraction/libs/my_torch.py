...
class LinearEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        print(x_wrd.shape)
        print(x_wrd[1])
        print(x_wrd.mean(dim=1).shape)
        return x_wrd.mean(dim=1)
...
class AttentionEncoder(nn.Module):
    ...
    def forward(self, inputs, world_vector=None):
        ...
        if world_vector is None:
            ...
            # use world with m
            x = x_wrd.matmul(self.M)
            x = x.matmul(world_vector.unsqueeze(1).unsqueeze(0))

        if self.b is not None:
            x += self.b
        if configuration.current_model == 'MATE' or configuration.current_model == 'ABAE':
            x = F.tanh(x)
        else:
            x = F.sigmoid(x)
        # print(x.shape)
    ...
    def get_world_vector(self, inputs):
        x_wrd = self.lookup(inputs)
        x_avg = x_wrd.mean(dim=1)
        return x_avg.mean(dim=0)

    def look_up(self, inputs):
        return self.lookup(inputs)

    def get_aspect(self, docs):
        doc_vector = self.lookup(docs)
        aspect_vector = doc_vector.mean(dim=1)
...
class FixedEncoder(nn.Module):
    ...
    def __init__(self, emb_size, num_aspects, aspect_size, fix_a_emb, document, weights=None, bias=True, use_enc=True, M=None, b=None):
        """Initializes the encoder using a [vocab_size x emb_size] embedding
        matrix. The encoder learns a matrix M, which may be initialized
        explicitely or randomly.
        ...
        self.aspect = nn.Parameter(torch.Tensor(num_aspects, aspect_size, emb_size))
        self.aspect.requires_grad = not fix_a_emb
        if weights is None:
            self.aspect.data.copy_(document)
        else:
            self.aspect.data.copy_(weights.matmul(document))
        ...
        if use_enc:
            ...
            self.M = nn.Parameter(torch.Tensor(emb_size, 1))
        if M is None:
            xavier_uniform(self.M.data)
        else:
            self.M.data.copy_(M)
    ...
    def forward(self, enc=None):
        """Forwards an input batch through the encoder"""
        return self.aspect.mean(dim=1)

    def get_attention_matrix(self):
        return self.M

    def get_world_vector(self):
        # x_avg = self.aspect.mean(dim=1)[0:-2]
        x_avg = self.aspect.mean(dim=1)
        # print(x_avg.shape)
        return x_avg.mean(dim=0)
...
class AspectEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, num_aspects, aspect_size, fix_a_emb, document, bias=True, use_enc=True, M=None, b=None):
        """Initializes the encoder using a [vocab_size x emb_size] embedding
        matrix. The encoder learns a matrix M, which may be initialized
        explicitely or randomly.
    ...
    def forward(self, enc=None):
        ...
        if enc is None:
            ...
            if configuration.current_model == 'MATE' or configuration.current_model == 'ABAE':
                ...
                x = F.sigmoid(x)
            a = F.softmax(x, dim=1)
            z = a.transpose(1, 2).matmul(self.aspect)
            z = z.squeeze()
            ...
            x = x.unsqueeze(1)
            x = x.unsqueeze(1)
            x = x.expand(enc.shape[0], self.num_aspects, 1, self.emb_size)

            x_wrd = self.aspect.transpose(1, 2)

            x = torch.matmul(x, x_wrd)
            if self.b is not None:
                x += self.b
            if configuration.current_model == 'MATE' or configuration.current_model == 'ABAE':
                x = F.tanh(x)
            ...
            a = F.softmax(x, dim=3)
            self.attention = a[:, 3, :, :]
            z = torch.matmul(a, x_wrd.transpose(1, 2))
            z = z.squeeze()
        if z.dim() == 1:
            return z.unsqueeze(0)
        return z
...
class DocumentKnowledgeAssistedAE(nn.Module):
    '''
        DocumentKnowledgeAssistedAE is a model based on AspectAutoencoder,
        which implements the calculation of aspect embeddings within the model.
        So that the model can train an aspect encoder (or attention model),
        instead of just training the aspect embedding vectors
    '''
    def __init__(self, vocab_size, emb_size, num_aspects=10, neg_samples=10,
            w_emb=None, a_weight=None, recon_method='centr', attention=False, instance_num = 0,
            bias=True, M=None, b=None, fix_w_emb=True, aspect_encoder='LSTM', aspect_size=10,
            document=None, fix_a_emb=False, constraint=1):
        """Initializes the autoencoder instance.
        ...
        Parameters:
            ...
            b (vector): bias vector for attention encoder (optional)
            fix_w_emb (bool): fix word embeddings throughout trainign
            aspect_encoder (str):  the method of constructing a aspect vector
                            - 'LSTM': using an LSTM encoder
                            - 'AttentionEncoder': using the AttentionEncoder
                            - 'plain': sum word vectors up
                            - 'muti-instance': muti-instance for one aspect
                            - 'init': use the a_emb
                            - 'init-fix': use the seed weight matrix and not change during training
            instance_num (int): for muti-instance encoder
            hidden_size (int): the size of hidden layer
        """
        super(DocumentKnowledgeAssistedAE, self).__init__()
        self.vocab_size = vocab_size
        self.emb_size = emb_size
        self.recon_method = recon_method
        self.attention = attention
        ...
        self.num_aspects = num_aspects
        self.neg_samples = neg_samples
        # self.documents = document.cuda()
        self.instance_num = instance_num

        if not attention:
            self.seg_encoder = nn.EmbeddingBag(vocab_size, emb_size)
            self.seg_encoder2 = AttentionEncoder(vocab_size, emb_size, bias, M, b)
        else:
            self.seg_encoder = AttentionEncoder(vocab_size, emb_size, bias, M, b)
        self.encoder_hook = self.seg_encoder.register_forward_hook(self.set_targets)
        ...
        if not attention:
            document_vec = self.seg_encoder2.look_up(document)
        else:
            document_vec = self.seg_encoder.look_up(document)

        self.constraint = constraint

        self.seed_w = None

        self.lin = nn.Linear(emb_size, num_aspects, bias=False)

        self.softmax = nn.Softmax(dim=1)
        if aspect_encoder == 'LSTM':
            self.aspect_encoder = nn.LSTM(emb_size, emb_size, num_aspects, batch_first=True)
        elif aspect_encoder == 'FixedEncoder':
            self.aspect_encoder = FixedEncoder(emb_size, num_aspects, aspect_size, fix_a_emb, document_vec, a_weight, bias, False, None, b)
        elif aspect_encoder == 'WeightEncoder':
            self.aspect_encoder = AspectEncoder(vocab_size, emb_size, num_aspects, aspect_size, fix_a_emb, document_vec, bias, False, None, b)
        elif aspect_encoder == 'AttentionEncoder':
            self.aspect_encoder = AspectEncoder(vocab_size, emb_size, num_aspects, aspect_size, fix_a_emb, document_vec, bias, True, M, b)
        elif aspect_encoder == 'plain':
            self.aspect_encoder = LinearEncoder(vocab_size, emb_size)
            self.aspect_encoder.set_word_embeddings(w_emb, fix_w_emb)
        else:
            self.aspect_encoder = None

    def forward(self, inputs, aspect_encoder, world_vector=None, batch_num=None):
        if self.training:
            self.cur_mask = self._create_neg_mask(inputs.size(0))
        else:
            print('in else!!!')
        ...
        if not self.attention:
            ...
            enc = self.seg_encoder(inputs, world_vector)

        x = self.lin(enc)
        a_probs = self.softmax(x)

        if aspect_encoder == 'LSTM':
            result, (out, c_n) = self.aspect_encoder(self.documents)
        elif aspect_encoder == 'AttentionEncoder':
            out = self.aspect_encoder(enc.detach())
        elif aspect_encoder == 'WeightEncoder':
            out = self.aspect_encoder()
        elif aspect_encoder == 'FixedEncoder':
            out = self.aspect_encoder()
        elif aspect_encoder == 'plain':
            out = self.aspect_encoder(self.documents)
        else:
            s = 'It has not been written!'

        if aspect_encoder == 'AttentionEncoder':
            r = a_probs.unsqueeze(1).matmul(out)
            r = r.squeeze()
        else:
            r = a_probs.matmul(out)

        return enc, r, a_probs
    ...
    def get_aspects(self):
        return self.aspect_encoder.get_aspect()

    def train(self, mode=True):
        super(DocumentKnowledgeAssistedAE, self).train(mode)
        if self.encoder_hook is None:
            self.encoder_hook = self.seg_encoder.register_forward_hook(self.set_targets)
        return self

    def eval(self):
        super(DocumentKnowledgeAssistedAE, self).eval()
        if self.encoder_hook is not None:
            self.encoder_hook.remove()
            self.encoder_hook = None
        return self
    ...
    def get_world_vector(self):
        return self.aspect_encoder.get_world_vector()

    def get_corpus_vector(self, inputs):
        return self.seg_encoder.get_world_vector(inputs)

    def set_linear(self):
        out = self.get_aspects()
        out = out / out.norm(p=2, dim=1, keepdim=True)
        self.lin.weight.data.copy_(out.detach())
        print(self.softmax(self.lin(out)))

    def get_attention_weight(self):
        return self.aspect_encoder.get_attention_weight()
