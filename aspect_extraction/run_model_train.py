...
def run_model(config, net, w2v_model, model, word2id, project_list, use_documents=True, use_test=False, data_size=0, tool=None):
    ...
    id2word = {}
    world_vector = []
    data, scodes, original = get_test_data(config, word2id, None)
    ...
    if model =='DKAAE-mi':
        ...
        sub_labels = []
        for i in range(0, config['aspects']-1):
            for j in range(config['instance_num']):
                sub_labels.append(i)
        sub_labels.append(config['aspects']-1)
        sub_labels = Variable(torch.from_numpy(np.array(sub_labels, dtype=np.int32)).long())
...
if __name__ == "__main__":
    ...
    if model_name == 'ABAE':
        ...
        run_model(config, net, model, model_name, word2id, project_list, use_documents=True, use_test=True, tool=None, data_size=0)